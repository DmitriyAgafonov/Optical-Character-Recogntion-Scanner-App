...
def scan_image(img, med_blur, canny, approx_param, crop_param,
               new_height=1800, new_width=1800):
    try:
        gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        resize_img = resize(gray_img, new_height, new_width)
        proc_img = preprocess(resize_img, med_blur, canny)
        fix_contour = get_contours(proc_img, resize_img, approx_param)
        warp_img = getWarp(resize_img, fix_contour, crop_param)
